...
### Performance Improvements
| Query | Before | After | Improvement | Method |
|-------|--------|-------|-------------|--------|
| Product list | 523 queries, 3.2s | 2 queries, 0.08s | 96% faster | select_related + prefetch_related |
| Order history | 1.5s | 0.12s | 92% faster | Query annotation + indexing |
...
-> Nested Loop (cost=0.43..8.46 rows=1) (actual time=0.025..0.026 rows=1)
   -> Index Scan using products_pkey (cost=0.29..8.30) 
   -> Index Scan using categories_pkey (cost=0.15..8.17)
...
### Migration Script
```python
# migrations/0001_optimize_queries.py
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            ...
            reverse_sql="DROP INDEX idx_order_user_status;"
        ),
    ]
```
```
...
- [x] Identified N+1 queries using Django Debug Toolbar
- [x] Applied select_related for ForeignKey/OneToOne
- [x] Applied prefetch_related for ManyToMany/reverse FK
- [x] Added database indexes for frequent lookups
- [x] Used only() to limit fields retrieved
- [x] Implemented query result caching
- [x] Converted count() queries to exists() where appropriate
- [x] Used bulk operations for multiple updates
...
- API performance → @django-api-developer
- Caching setup → @deployment-engineer
- Database migrations → @database-optimizer
- Performance testing → @performance-engineer
- Query profiling → @debugger

## Best Practices
- Profile before optimizing (use Django Debug Toolbar)
- Use select_related for forward ForeignKey/OneToOne
- Use prefetch_related for reverse ForeignKey/ManyToMany
- Add indexes for fields used in filters and ordering
- Use database views for complex read-only queries
- Implement query result caching strategically
